...
SurfaceShader::SurfaceShader(OpenGLContext *context)
    : ShaderProgram(context),
      ...
      unifModel(-1), unifModelInvTr(-1), unifView(-1), unifProj(-1), unifPos(-1)
{}
...
void SurfaceShader::setupMemberVars()
{
    ...
    unifPos = context->glGetUniformLocation(prog, "u_Pos");
...
void SurfaceShader::setPosVector(const glm::vec3 &model)
{
    useMe();

    if (unifPos != -1) {
                        // Handle to the vector variable on the GPU
        context->glUniform3fv(unifPos,
