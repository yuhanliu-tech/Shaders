...
void MyGL::paintGL()
{
    ...
    // update value to camera world space pos
    mp_progSurfaceCurrent-> setPosVector(m_camera.eye);
...
void MyGL::createShaders()
{
    ...
    std::shared_ptr<PostProcessShader> inverted = std::make_shared<PostProcessShader>(this);
    inverted->create(":/glsl/post/passthrough.vert.glsl", ":/glsl/post/inverted.frag.glsl");
    m_postprocessShaders.push_back(inverted);

    std::shared_ptr<PostProcessShader> hatches = std::make_shared<PostProcessShader>(this);
    hatches->create(":/glsl/post/passthrough.vert.glsl", ":/glsl/post/hatches.frag.glsl");
    m_postprocessShaders.push_back(hatches);
